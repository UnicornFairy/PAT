...
int main() {
    double x,i;  //x是车速,i是限速
    cin >> x >> i;
    if(x < (int)(i * 1.1)){
        cout << "OK";
    }else if(x >= (i * 1.1) && x <= (1.5 * i)){
        cout << fixed << setprecision(0) << "Exceed " << (x / i) * 100 - 100 << "%. Ticket 200";
    }else{
        cout << fixed << setprecision(0) << "Exceed " << (x / i) * 100 - 100 << "%. License Revoked";
    }
    return 0;
}
